...
export default withIronSessionApiRoute(
	async function handler(request, response) {
		...
		if (
			receiver &&
			Number.isInteger(Number(receiver)) &&
			Number(receiver) > 0
		...
		if (typeof sender !== 'undefined') {
			if (
				...
				database.get(
					`SELECT * FROM users WHERE username = ${sender}  OR id = ${sender}`,
					(error, sender) => {
						if (error) throw error
						if (sender) {
							database.get(
								`SELECT * FROM users WHERE username = ${receiver} OR id = ${receiver}`,
								(error, receiver) => {
									if (error) throw error
									if (receiver) {
										if (sender.username !== receiver.username) {
											if (sender.balance >= amount) {
												database.run(`UPDATE users SET balance = ${sender.balance - amount} WHERE id = ${sender.id}`,
													(error, results) => {
														if (error) throw error
													})
												database.run(`UPDATE users SET balance = ${receiver.balance + amount} WHERE id = ${receiver.id}`,
													(error, results) => {
														if (error) throw error
													})
												let date = new Date()
												...
												database.run(`INSERT INTO transactions (senderId, receiverId, amount, timestamp) VALUES (${sender.id}, ${receiver.id}, ${amount}, ${JSON.stringify(date)})`,
													(error, results) => {
														if (error) throw error
													})
